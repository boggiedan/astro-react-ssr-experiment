# Node.js + Preact SSR (Vanilla)

Minimal Node.js HTTP server with Preact SSR using abstract 3-layer architecture.
...
- **Vanilla Node.js + Preact** with worker pool and traditional SSR modes
...
- Preact components (Header, SimpleReact, DataDisplay, ComplexChart, DataProcessor)
...
curl "http://localhost:3001/api/user?delay=100"
curl "http://localhost:3001/api/posts?count=5"

# Test pages
curl http://localhost:3001/                    # Home page
curl http://localhost:3001/test/simple         # Simple test (0 API calls)
curl http://localhost:3001/test/api-heavy      # API-heavy test (3 parallel API calls)
curl http://localhost:3001/test/cpu-intensive  # CPU-intensive test (2500 items processing)
curl http://localhost:3001/test/mixed          # Mixed test (API + CPU work)
```
