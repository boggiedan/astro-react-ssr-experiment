 ...
 * Worker Pool Manager
 *
 * Manages a pool of worker threads for React rendering using Piscina.
 * Workers ONLY execute renderer functions - all HTTP handling and data fetching stays on main thread.
 *
...
class WorkerPoolManager {
  ...
  async initialize(): Promise<void> {
    ...
    // Load compiled worker from dist/server
   ...
   * Worker ONLY executes renderer function - all data fetching stays here on main thread.
   */
  async render(input: RenderInput): Promise<RenderOutput> {
    ...
    try {
      ...
      const output: RenderOutput = await this.pool.run(input);
